...
7. **The junction control is gated in two tiers.** *Tier 1, pre-flight, LOCAL, blocks the submit-ack:* a background→background control — same host, same phase, same insert length, real DNA from another window's same offsets — must sit within a **measured** null band, `dev(junction) ≤ dev(null) + 3·SE_H0`, where the null is the same classifier separating two disjoint sets of *unspliced* windows and `SE_H0` is Hanley–McNeil's null standard error. A **must-fire** clause runs alongside it: the probe must separate the decoy arm from its control, or the run is reported **underpowered**, never *clean* — "the junction is invisible" and "the probe sees nothing" otherwise produce identical numbers. Matchedness is **asserted** from the emitted sequences (including that the control is not merely the unspliced host) and the generator is **sabotage-tested**. *Tier 2, post-run, model-side:* the trained checkpoint scores the same arms and must satisfy `AUROC(decoy vs control) > AUROC(control vs plain)` — **the decoy must beat its own junction null**. Deliberately threshold-free: pinning an absolute bar is how P2-07 certified a perturbation whose effect was smaller than its own null.
...
**Measured on the shipped artifacts (`reports/p2/junction_control.json`, seed 20260721, real pools at `029b432`).** Arms: **4,999** embedded / **2,804** per unspliced arm, from **15,606** unique admitted windows (**102** byte-identical duplicate windows dropped — the pool carries the same DNA under distinct `candidate_id`s, and a duplicate spanning two arms leaks across CV folds exactly like a shared host).

| k | null | junction | decoy content | SE_H0 | clauses |
|---|---|---|---|---|---|
| 4 | 0.5060 | **0.5179** | 0.7394 | 0.0077 | 4/4 |
| 6 | 0.5115 | **0.5058** | 0.7599 | 0.0077 | 4/4 |

`overall_pass = true`, and the gate requires **every** k to pass — a cue invisible at k = 4 but readable at k = 6 is still a cue, and reporting the friendlier one is the "flattering unit" failure A5 names. Realized composition: **15,708** plain + **4,999** embedded (`dinuc_shuffled` 2,000 + `structured_rna` 2,999; 1 masked) = **20,707** admitted negative records → shares **75.9 % / 9.7 % / 14.5 %**, with **2,008** decoys refused by pins 4–5. At f = 10/11 that is ≈ **4.01** draws per record per epoch.

**Scope limits, disclosed rather than buried.**
- **Tier 1 is a *composition* bound, not a certificate.** A k-mer frequency over a 128-nt window dilutes a single-position discontinuity that a convolutional or state-space receptive field could read. It removes the compositional cue as an *explanation*; tier 2 is what tests the model actually trained.
- **The band is derived from a single null draw**, so it inherits that draw's noise. It is stable at the shipped n (2,804–4,999) but **not** at small n: at n = 90 the 256-column featuriser overfits enough to separate two *unspliced* host sets at 0.576 while the null draw landed at 0.504, and the junction arm then read 0.66 with nothing about the junction having changed. The CI fixture runs at n = 600 for that reason, recorded in the test rather than tuned silently.
