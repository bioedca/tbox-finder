...
**Block power is reported at both resampling units.** **103 clusters / 26 orders** (both ≥ 20). Reporting only the flattering unit is how an underpowered arm reads green, so `build_report` grades at the **weakest** unit and emits both.
