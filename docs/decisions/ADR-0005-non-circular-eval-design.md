...
- **Status:** **Accepted (user sign-off 2026-07-21; CLAUDE.md §7 item 2)** — the four contested pins (per-class shares, `leader_decoy`, the junction-control rule, class weighting) were each selected from drafted alternatives. Amends **D14** with the *training-mix composition* D14 left open, and re-scopes one D14 clause (the retained leader pool). Pins no existing value: D7's 100:1 benchmark decoy prevalence, `MIN_REAL_HOMOLOG_N = 20`, and A4/A5 are untouched. Consumed by **P2-10e** (this run is its round-0 parent) and by **P4/GATE-1**. Committed atomically with `data/embedding.py`, `eval/junction_probe.py`, `scripts/measure_junction_control.py`, their unit + generator-sabotage + ml gates, and `reports/p2/junction_control.json`.
