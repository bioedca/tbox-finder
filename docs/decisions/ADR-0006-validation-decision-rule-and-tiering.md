...
## Amendment A2 — The AnyHelixCriterion operator + the min_sequences covariation-power floor (P2-10e, 2026-07-26)

- **Status:** **Accepted (user sign-off 2026-07-26; CLAUDE.md §7 item 2, bioedca).** Amends **D2 / D11 / D7** (the covariation spare-rule leg). Pins the two owed decisions `covariation.py` labels *"an ADR-0006 decision owed at P2-10e."*

**Pin 1 — AnyHelixCriterion = TOTAL_ACROSS_HELICES** (RECOMMENDED value; the looser, more Tier-2N-protective reading: ≥ 2 covarying pairs **summed across** conserved helices, `sum` over helices of `nbp_cov`). Rationale: the spare rule exists to keep real Tier-2N loci **out** of the hard-negative-mining pool; the more-sparing operator is the safe direction. The in-repo sweep shows the two operators agree at high power (0 % divergence at 5–8 and 60 seqs) and disagree on **up to 33 %** of low-power alignments (10/12/15/20/30 seqs: 17/25/17/33/8 %) — the regime a real homolog set occupies — and **every divergent case is one the looser operator spares and the stricter one mines.** Consistent with D11/D14's *"directionally-bounded Tier-2N discovery sensitivity, not an invalid generalization claim."* Literature support for helix-level covariation aggregation raising sensitivity without sacrificing specificity: Rivas 2023, *PLoS Comput Biol* (DOI:10.1371/journal.pcbi.1011262, accessed 2026-07-26). [The operator is a **policy** choice on mining aggressiveness; the literature supports helix-level aggregation in general, it does not by itself dictate the sum-vs-max reading.]

**Pin 2 — min_sequences = 20** (RECOMMENDED value; = `MIN_REAL_HOMOLOG_N`). Below this alignment-depth floor the covariation (a)-leg is declared `unavailable` → the candidate is **SPARED** (fail-closed), never `failed` → mined. Aligns the covariation power floor with the already-pinned D18/A1 homolog-N floor. In-repo sweep: **0/12** seeded subsamples pass at 5–8 sequences vs **12/12** at 60. §10.1 evidence gate (≥ 2 agreeing sources; high-stakes — it underpins the Tier-2N spare rule): covariation power depends on alignment depth **and** sequence variability, and below adequate power covariation cannot be detected regardless of whether a conserved structure is present, so a floor is methodologically required and its role is to route low-power alignments to *"can't tell"* rather than force a verdict — Rivas, Clements & Eddy 2017, *Nat Methods* (PMID:27819659, DOI:10.1038/nmeth.4066); Gao, Yang & Rivas 2022, *IUBMB Life* (DOI:10.1002/iub.2694); Rivas 2021, *WIREs RNA* (DOI:10.1002/wrna.1649). All accessed 2026-07-26.

**Scope guard (§10.3).** These two pins gate ONLY the per-candidate covariation (a)-verdict inside a round — they do **NOT** block a P2-10e *submit* (the backend is fail-closed: an unscored (a)-leg → `unavailable` → spared, never silently mined). The (a)-verdict is **additionally** gated by the homolog-search MSA supply (D7 / A1 substrate: homolog-DB search + CM-free de-novo MSA per candidate), which is downstream infrastructure — so even with these pins, no (a)-verdict is producible until that leg lands.

**Scope-guard correction (P2-10e ENCODE, 2026-07-26).** The sentence above ("do NOT block a P2-10e submit") describes only the **per-candidate** fail-closed sparing and is incomplete: the shipped P2-07 **round-level** readiness gate (`spare_rule.mining_round_readiness`, invoked first by `hard_negative.mine_round`, which **raises**) refuses a round entirely unless **≥ 1 protective spare-rule backend** — covariation-(a) or synteny-(c) — is available; relaxed-architecture-(b) alone is `False` on every Tier-2N locus (D9 row 5). At P2 neither protective backend exists (the D7/A1 per-candidate MSA supply is not built — only the homolog-search *target DB* is, P2-10c′-homologdb; and there is no synteny backend), so **every P2-10e round is refused at readiness** and the N=4 RUN (ADR-0005 A9) is blocked until that supply lands. Correspondingly, a round's covariation-(a) *availability* must be gated on **MSA-producibility**, not on R-scape being *installed*: `covariation.backend_available` (and `round_backend_availability`) answers only "is R-scape installed", and a round declared available on that basis would run as a **no-op** (no MSAs ⇒ every candidate spared ⇒ zero mined) yet report success — the exact degradation the readiness gate exists to prevent but does not catch. `mining/mine_round.py::build_round_availability` therefore gates `any_helix_rscape = rscape_installed AND msa_supply_available`.

**As encoded (P2-10e ENCODE).** `mining/covariation.py::covariation_spare_status(alignment, *, min_sequences=MIN_REAL_HOMOLOG_N)` is the single production call site: it pins `AnyHelixCriterion.TOTAL_ACROSS_HELICES` (Pin 1) and defaults `min_sequences` to `MIN_REAL_HOMOLOG_N` (Pin 2), and fail-closes to `STATUS_UNAVAILABLE` when `alignment is None` (no per-candidate MSA) or on `CovariationBackendError` (R-scape absent/unpinned, or depth below the floor). The round-level MSA-producibility gate is `mining/mine_round.py::build_round_availability` (above).

**Not amended.** D7's homolog-search-inclusion thresholds (P6-frozen, D17); A1's target-DB source + count (2,500 GTDB R232 reps).

**Cross-reference impact.**
- **ADR-0005 A9 / D14** — the covariation (a)-leg the mining spare rule + Tier-2N halt depend on; A9's N=4 RUN is blocked at the readiness gate this amendment's correction documents.
...
## Amendment A4 — D3's criterion-(b) element localization is instantiated by the D7/A3 de-novo comparative consensus, superseding the unreachable R2DT / class-I-templated `cmalign` route (P3-15′-d, 2026-08-07)

- **Status:** **Accepted (user sign-off 2026-08-07; CLAUDE.md §7 item 2, bioedca).** Records which instrument supplies **D3**'s `named_elements_present` / `ncca_bulge_detected` inputs. **No D3 predicate logic, no D6/D7/D8 P6-frozen value, and no D9 routing row is amended.** Requires **no** ADR-0002 amendment: `envs/locarna.yml` (`locarna` 2.0.1, ADR-0002 A13) and `envs/rscape.yml` (A11) are already pinned and locked.

**What D3 names, and why it cannot be honoured literally.** D3's parenthetical is *"(R2DT 2.0 / `cmalign` on the host cross-check; §13.2)"* — two named instruments. Both fail, for different reasons:

1. **R2DT is unreachable under ADR-0002's regime.** Measured 2026-08-07: `api.anaconda.org` returns *"could not be found"* for `bioconda/r2dt`, `conda-forge/r2dt` **and** `bioconda/traveler` (its renderer), while `bioconda/infernal` resolves on the same call — so the probe is powered, not silently broken. ADR-0002 D1 records **no container runtime on the cluster** (no apptainer/singularity/docker/podman). R2DT 2.0 itself lists *"Facilitating R2DT installation via conda"* as **future** work [PMID:39921562].
2. **R2DT would supply no independent structure even if installable.** Its Rfam path *is* the `cmalign` path: *"For sequences matching templates, the 2D structure is predicted using the Infernal `cmalign` program by aligning the input sequences to the template covariance models"* [PMID:39921562]; *"R2DT uses the Rfam covariance models built from the Rfam seed alignments"* [PMID:34108470]. Traveler adds standardised **layout**, not a CM-free structure source; R2DT ships an Rfam-derived T-box template at `data/rfam/RF00230/`; its "template-free" mode **takes a dot-bracket the caller supplies**. On a locus with no template hit it *"will not attempt to draw a structure"* [PMID:34108470] — the same blindness as `cmalign`, not a remedy for it.
3. **The `cmalign`-vs-RF00230 route is blind precisely at the flagship class, and needs a second valueless delegation.** RF00230 (Rfam 15.1, `num_seed` 48) is described in terms of **transcription** antitermination; the class-II corpus required a **custom** covariance model, not RF00230 [PMID:32882008], and ultrashort-Stem-I variants *"lack the key motifs present in the canonical transcriptional T-box systems"* [DOI:10.1002/iub.2098; PMID:25583497]. Localizing (b)'s elements by `cmalign` further requires knowing whether a candidate clears **D8**'s RF00230 alignability cutoff — which **D17** freezes at P6 with **no value** — so the route needs D6 *and* D8, not D6 alone. PRD §18.2 already names *"orthogonal validation re-imposes the CM frame"* as a project risk whose pinned mitigation is the de-novo route. ⚠ **No source quantifies an RF00230 recall/bit-score deficit on class-II or ultrashort loci; no such number is asserted here (§10.3).**

**Pinned instantiation.** D3's element inputs are localized from the **A3 per-candidate `mlocarna` `#=GC SS_cons` comparative consensus** of the candidate's own homolog set — the same artifact that already supplies (a) — together with the candidate's own primary sequence. Hard constraints: **never** RF00230, **never** an Rfam/class-I template, **never** the Stage-1/Stage-2 model's own output. The operational predicate remains exactly as D3 already requires — **frozen on the held-out canonical set and unit-tested**. This amendment pins the **instrument and its anti-circularity constraints only, not an algorithm**; no localization rule is fabricated here (§10.3).

**D6's threshold is supplied, not frozen.** `ultrashort_relax`'s short-Stem-I nt threshold ships **keyword-required with no default**, supplied per round and recorded in provenance — the discipline already used by `--strand-policy` (P3-15′-b), `substrate_prescan.scan_shard`'s `score_threshold` (ADR-0005 A8), and criterion (c)'s three required parameters (P3-15′-c-ii). This **does not consume D17's P6 delegation**; the P6 freeze stands unchanged. The relaxation only ever makes (b) *more* likely to pass ⇒ more sparing ⇒ the fail-closed direction, so an imperfect value errs safe.

**⚠ Disclosed cost — (a) and (b) cease to be independent disjuncts.** (b) now reads the same MSA (a) does, so it is computable only where that MSA is buildable (A2's `min_sequences = 20` floor). Where the MSA is unavailable, (b) is `unavailable` and **spares** (fail-closed). ADR-0005 D14 describes its three model-independent disjuncts as independent; **two of them now share a supply**, and any report resting on their conjunction must say so. This is a real weakening of the rule's redundancy, disclosed rather than absorbed.

**⚠ Measured context for what (b) can still change.** At sign-off the mining ceiling is already bounded **without** (b): criterion (c) is `failed` on **96 of 941** candidates (`546 passed / 96 failed / 299 unavailable`, reproduced from the committed config), and intersecting that with the 941-candidate Stage-2 posteriors leaves **69–80 candidates** (7.3–8.5 %) failing both across every Stage-2 threshold in [0.5, 0.99]. Because mining is a conjunction, **(b) can only lower that number, never raise it**, and by D9 row 5 it contributes **zero** Tier-2N protection (`TIER2N_PROTECTIVE_DISJUNCTS` excludes it, frozen with no override). (b) is built so the conjunction can complete and so canonical-architecture candidates that (a) and (c) both missed retain a model-independent guard — not to increase yield.

**Not amended.** D3's base predicate and its two relaxations; D3's prohibition on keying (b) to a `cmalign`-vs-RF00230 bit-score (**strengthened** — no `cmalign` is on (b)'s path at all); D6/D7/D8's P6-frozen values and D17's delegation map; D9's routing, including row 5's `(c)✓ ∧ (a)✓ ∧ (b)✗` Tier-2N definition; D5/D8's **specifier**-localization path (unchanged, including its `cmalign`/R2DT wording — this amendment governs criterion (b) only); A1's target DB; A2's operator and `min_sequences = 20` floor; A3's aligner. **R2DT as a §17 publication-figure deliverable is untouched and separable from the predicate.**

**§10.1 evidence** (accessed 2026-08-07; article metadata + full text via PubMed/PMC, Rfam 15.1 API, r2dt-bio/R2DT README). R2DT internals + distribution: PMID:34108470 [DOI:10.1038/s41467-021-23555-5] · PMID:39921562 [DOI:10.1093/nar/gkaf032]. RF00230's class-I scope / class-II custom CM: PMID:32882008 [DOI:10.1093/nar/gkaa721] · Rfam RF00230 rel. 15.1. Ultrashort / non-canonical Stem I: PMID:25583497 [DOI:10.1073/pnas.1424175112] · [DOI:10.1002/iub.2098] · PMID:29581302 [DOI:10.1073/pnas.1721254115].

**Cross-reference impact.** **D3** (instrument pinned) · **A3** (its `#=GC SS_cons` becomes load-bearing for (b) as well as (a), and must be **promoted out of the `$BUILD/work` node-local scratch where `slurm/p2/mine_round_producer.sbatch` currently destroys it**) · **D6/D17** (threshold supplied per round, P6 freeze untouched) · **D8** (its alignability cutoff is no longer on (b)'s path) · **D9 row 5** (unchanged) · ADR-0005 **D14** (its "three independent model-independent disjuncts" framing now carries the shared-supply caveat above) · ADR-0005 **A10** (the (a) producer's pinned envelope now also sizes (b)'s supply) · **ADR-0002** (no amendment required; A11/A13 envs suffice) · **PRD §5, §6, §13.2, §13.3(b)** (prose updated in the same step to name the de-novo consensus as (b)'s instrument).

- **Amendment A4 sign-off:** ☑ recorded 2026-08-07 (bioedca), CLAUDE.md §7 item 2 — D3's criterion-(b) element localization instantiated by the A3 de-novo comparative consensus (R2DT measured unreachable *and* CM-derived; RF00230 class-I-scoped; D8's cutoff valueless); D6's threshold supplied keyword-required-no-default, P6 freeze untouched; (a)/(b) supply-dependence disclosed. No D3/D6/D7/D8/D9/A1/A2/A3 value changed.
