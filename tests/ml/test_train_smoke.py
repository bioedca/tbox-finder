...
def test_committed_production_report_records_its_audited_dirty_tree() -> None:
    """P2-10d'-b job 680 — the committed production report whose tree was legitimately dirty.
    ...
    This report supersedes the P2-09 job-671 one (schema_version 1), which PREDATED the
    `git_dirty_paths` field: it could not re-confirm its own audit from the recorded fields,
    so `_provenance_complete` re-derived False against a stored True and this test used to
    pin that divergence. This report is written by the post-P2-10 code that RECORDS the dirty
    paths, so the audit is machine-checkable rather than manual — the only legitimate dirt is
    the LFS-staged split table under `data/`, which is exactly why `_provenance_complete`
    now re-derives True and the stored gate is self-consistent. When this file is
    regenerated, this test must be updated consciously.
    """
    ...
    # The report now RECORDS its dirty paths, so the audit is machine-checked, not manual:
    # the ONLY legitimate dirt is the LFS-staged split table under data/. A tracked non-data
    # path here would mean git_sha does not describe the code that ran (false provenance, §11).
    dirty_paths = prov["git_dirty_paths"]
    assert (
    ...
    # Every dirty path is data-only, so the stored gate and the re-derivation now AGREE
    # (the P2-09 divergence is gone): pinning BOTH True makes a silent drift back to
